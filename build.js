...
async function buildFromSource(blob, mods) {
    ...
    // Patch loader.lua — inject stubs at top of initLoader, keep loadMods call as-is
    // (loadMods is a local function so pcall across scope doesn't work)
    for (const lp of ['SMODS/preflight/loader.lua', 'SMODS/src/preflight/loader.lua']) {
        ...
        const patch =
            ...
            '        }\n' +
            '    end'
        lc = lc.replace('function initLoader()', patch)
        zipfile.file(lp, lc)
