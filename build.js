...
async function buildFromSource(blob, mods) {
    ...
    if (mods["Dump from Lovely"]) {
        ...
        // Flatten lovely/SMODS/ preflight files to SMODS/
    ...
    for (const [name, mod] of Object.entries(mods)) {
        ...
        const tomls = []
        if (mod["lovely.toml"]) {
            tomls.push({ toml: mod["lovely.toml"], path: "lovely.toml" })
        }
        if (mod["lovely"]) {
            for (const [name, patch] of Object.entries(mod["lovely"])) {
                if (name.endsWith(".toml")) {
                    tomls.push({ toml: patch, path: "lovely/" + name })
                }
            }
        }
    ...
    // Find SMODS mod folder and copy src files
    ...
    if (smodsFolderName) {
        ...
        // Add dont_patch.txt to prevent duplicate SMODS detection
        zipfile.file('Mods/' + smodsFolderName + '/dont_patch.txt', 'Loaded via Lovely dump.')
        console.log('Added dont_patch.txt to Mods/' + smodsFolderName + '/')
    }
    // Fix goto in ALL lua files after everything is in the zip
    await fixGotoInZip(zipfile)
    // Patch loader.lua — stub missing globals before loadMods runs
    // Use string concat to avoid template literal issues with Lua code
    for (const lp of ['SMODS/preflight/loader.lua', 'SMODS/src/preflight/loader.lua']) {
        const loaderFile = zipfile.file(lp)
        if (!loaderFile) continue
        let lc = await loaderFile.async('string')
        const loaderPatch = 'function initLoader()\n' +
            ...
            '        }\n' +
            '    end'
        lc = lc.replace('function initLoader()', loaderPatch)
        const loadModsPatch = 'local _lm_ok, _lm_err = pcall(loadMods, SMODS.MODS_DIR)\n' +
            '    if not _lm_ok then print("loadMods error (non-fatal): " .. tostring(_lm_err)) end'
        lc = lc.replace('loadMods(SMODS.MODS_DIR)', loadModsPatch)
        zipfile.file(lp, lc)
        console.log('Patched loader.lua at', lp)
    }
    ...
    // Web nativefs at root — require('nativefs') always finds web version first
    ...
    // Replace nativefs in all mod folders
    for (const path of Object.keys(zipfile.files)) {
    ...
    if (!zipfile.file("web_patched") || mods["Dump from Lovely"]) {
        progress_bar.value = "60"
        {
            ...
            // Fix SMODS globals and nativefs after preflight
        ...
        {
            // Patch game.lua shader loading — wrap newShader in pcall
        ...
        {
            // Disable fullscreen in conf.lua
